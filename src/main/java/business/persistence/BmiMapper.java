...
public class BmiMapper {
        ...
        try (Connection connection = database.connect())
        {
            ...
            try (PreparedStatement ps = connection.prepareStatement(sql, Statement.RETURN_GENERATED_KEYS))
            {
                ...
                for (Integer hobbyId : hobbyList) {
                    intertIntoLinkHobbyBmiEntry(bmiEntryId, (int)hobbyId);
                }
            }
    ...
    public void intertIntoLinkHobbyBmiEntry(int bmiEntryId, int hobbyId) throws UserException{

        try (Connection connection = database.connect())
        {
            String sql = "INSERT INTO link_bmi_hobby " +
                    "(`hobby_id`," +
                    "`bmi_entry_id`) " +
                    "VALUES (?,?);";
